...
def generate_dataset_file(dataset_name, dataset_root_path, output_file_path, compression_level='c23', perturbation = 'end_to_end'):
    ...
    ## Note: videos in Celeb-DF-v1/2 are not in the same format as in FaceForensics++ dataset
        ...
        for folder in os.scandir(dataset_path):
            ...
            # 遍历该文件夹下的视频文件（mp4 文件）
            for video_path in os.scandir(os.path.join(dataset_path, folder.name)):
                if video_path.is_file() and video_path.name.endswith('.mp4'):  # 只处理 mp4 文件
                    video_name = video_path.name.split('.mp4')[0]  # 获取视频名称（去掉扩展名）
    ...
    ## DFDC dataset
        ...
        for folder in os.scandir(dataset_path):
            ...
            if folder.name in ['test']:
                ...
                for video_path in os.scandir(os.path.join(dfdc_train_part)):
                    if video_path.is_file() and video_path.name.endswith('.mp4'):
    ...
    # mydata
        ...
        dataset_path = os.path.join(dataset_root_path, dataset_name)
        dataset_dict[dataset_name] = {f'{dataset_name}_Real': {'train': {}, 'test': {}, 'val': {}},
                                f'{dataset_name}_Fake': {'train': {}, 'test': {}, 'val': {}}}
        for index, folder in enumerate(os.scandir(dataset_path)):
            if not os.path.isdir(folder):
                continue
            elif folder.name in ['fake']:
                for video_path in os.scandir(os.path.join(dataset_path, folder.name, 'frames')):
