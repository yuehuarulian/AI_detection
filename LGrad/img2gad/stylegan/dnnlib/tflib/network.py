...
class Network:
    ...
    def run_img2grad(self, imgdir, 
        out_dir, # 输出目录，用于保存生成的图像梯度
        return_as_list  = False,    # 是否将结果作为列表返回。True 表示返回 NumPy 数组列表，False 表示返回单个 NumPy 数组，或元组（如果有多个输出）
        print_progress  = False,    # 是否在控制台打印进度，适用于大规模输入数组
        minibatch_size  = 1,     # 最大的最小批量大小，None 表示禁用批量处理
        num_gpus        = 1,        # 使用的 GPU 数量
        **dynamic_kwargs):          # 传递给网络构建函数的其他关键字参数
        
        # 对图像进行归一化处理，将像素值范围从 [0, max] 缩放到 [0, 255] 之间
        def normlize_np(img):
            img -= img.min()
            if img.max()!=0:
        ...
        imgnames_list = get_imglist(imgdir) # 获取图像文件路径列表
        num_items = len(imgnames_list)
        print(f'From {imgdir} read {num_items} img')
        
        if minibatch_size is None:
            minibatch_size = num_items # 如果未指定批量大小，则将其设置为图像总数
        key = str([list(sorted(dynamic_kwargs.items())), num_gpus]) # 生成图计算缓存的键
        ...
        for mb_begin in range(0, num_items, minibatch_size):
            ...
            imgs_np      = read_batchimg(imgname_list) # 读取并处理当前批次的图像
            ...
            for imgname, img_np, grad in zip(imgname_list, imgs_np, mb_out[-1]):
                if out_dir is not None: 
                    ...
                    cv2.imwrite(tmppath, normlize_np(grad.transpose(1,2,0))[...,::-1]) # 保存梯度图像
