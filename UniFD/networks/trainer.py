...
class Trainer(BaseModel):
    ...
    def __init__(self, opt):
        ...
        torch.nn.init.normal_(self.model.fc1.weight.data, 0.0, opt.init_gain)
        torch.nn.init.normal_(self.model.fc2.weight.data, 0.0, opt.init_gain)
        if opt.fix_backbone:
            params = []
            for name, p in self.model.named_parameters():
                if  name=="fc1.weight" or name=="fc1.bias" or name=="fc2.weight" or name=="fc2.bias": 
                    params.append(p) 
                else:
    ...
    def load_checkpoint(self, checkpoint_path):
        ...
        print(f"恢复训练，总步数: {self.total_steps}")
