...
'''data_mode感觉可以省略掉，没有特别大的用处'''
'''需要提前单独的抽帧操作，可能需要另外一个单独的文件将视频数据集抽成图片数据集，然后再使用该类'''
class RealFakeDataset(Dataset):
    def __init__(self, opt):
        ...
        if opt.data_mode == 'ours':
            ...
            real_list += get_list( os.path.join(opt.wang2020_data_path,temp), must_contain='0_real' )
            fake_list += get_list( os.path.join(opt.wang2020_data_path,temp), must_contain='1_fake' )
        elif opt.data_mode == 'ffpp':
            temp = 'train' if opt.data_label == 'train' else 'val'
            # Collect from all folders under 'train' or 'test'
            real_list, fake_list = [], []
            base_path = os.path.join(opt.wang2020_data_path, temp)
            for folder in os.listdir(base_path):
                real_list += get_list(os.path.join(base_path, folder), must_contain='0_real')
                fake_list += get_list(os.path.join(base_path, folder), must_contain='1_fake')
        ...
        if not opt.isTrain and opt.no_resize:
            ...
            rz_func = transforms.Resize(self.opt.loadSize)
