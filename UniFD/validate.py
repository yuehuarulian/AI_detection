...
#为了方便验证，又实现了一个功能类似的dataloader，省略了一些图像裁剪，模糊等处理，除非有明确的参数
...
if __name__ == '__main__':
    ...
    if (opt.real_path == None) or (opt.fake_path == None) or (opt.data_mode == None):
        print("dataset path or data mode not provided")
        exit()
    else:
    ...
    for dataset_path in (dataset_paths):
        ...
        with open( os.path.join(opt.result_folder,'acc1.txt'), 'a') as f:
            f.write(str(round(r_acc1*100, 2))+'  '+str(round(f_acc1*100, 2))+'  '+str(round(acc1*100, 2))+'  '+str(round(best_thres,2))+'\n' )
