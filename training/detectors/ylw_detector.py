...
# date: 2024-1026
# description: Class for the YLWDetector
...
@DETECTOR.register_module(module_name='ylw')
class YLWDetector(AbstractDetector):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.num_classes = config['backbone_config']['num_classes']
        self.encoder_feat_dim = config['encoder_feat_dim'] # 512
        self.half_fingerprint_dim = self.encoder_feat_dim//2 # 256
        ...
        # head
        specific_task_number = config['specific_task_number']  # default: 5 in FF++
        self.head_spe = Head(
    ...
    def features(self, data_dict: dict, phase_fea) -> torch.tensor:
        ...
        cat_data = torch.cat((data_dict['image'], phase_fea), dim=1)
        # encoder
    ...
    def forward(self, data_dict: dict, inference=False) -> dict:
        ...
        features = self.features(data_dict, phase_fea) # 图片加了一个相位的特征
        ...
        f_spe, f_share = self.classifier(forgery_features) # f_spe: specific fingerprint, f_share: commonprint
